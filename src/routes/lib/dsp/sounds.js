...
export function white_noise( sample_size ) {
  let data_noise = null;
...
export function spectral_filter( audio, filter, overlap = 8, window_size = 2048 ) {
  ...
  llog( frames );
	...
	frames.forEach(( frame, i ) => {
		ffts.push ( (new fft.ComplexArray( mul( frame, w ))).FFT() );
	});
  llog( ffts );
...
export function pink_noise( sample_size = 44100 ) {
	// Generate noise.
